  ...
  function forContext(ctx) {
    ...
    function weeklyPickAssignment(rows, onHand, previousPn, weeklyBurnByPn, bufWeeksLocal, globalCaps, cols, currentColIdx) {
      ...
      for (const cand of candidates) {
        const { score, horizonMinRaw } = weeklyScore(cand, rows, onHand, previousPn, weeklyBurnByPn, bufWeeksLocal, globalCaps, baselineMinCover, remainingWeeks, baselineProjection);
        if (best === null) {
          best = cand; bestScore = score; bestHorizonMin = horizonMinRaw;
          continue;
        }
        if (score > bestScore + WEEKLY_TIEBREAK_EPS) {
          best = cand; bestScore = score; bestHorizonMin = horizonMinRaw;
        } else if (Math.abs(score - bestScore) <= WEEKLY_TIEBREAK_EPS) {
    ...
    function simulateWeekly(rows, cols, slots, globalCaps, rateByPn) {
      ...
      for (const [iso, wk] of weekDataByIso.entries()) {
