...
(function (root, factory) {
  ...
  function forContext(ctx) {
    ...
    // Warn-once for a LOCKED row that carries no stored qty map. Such a
    // row cannot replay verbatim (there is nothing to replay) and is
    // live-sized this pass. In the browser js/25 backfills the persist
    // once so it freezes from then on; on the server this is just a
    // trace. Once per week-iso, module-level so the optimizer's tens of
    // thousands of simulate() calls do not repeat it. Console only --
    // the pure-math contract holds.
    const _lockedNoQtyWarned = new Set();
    function _warnLockedNoQty(iso, pn) {
      if (_lockedNoQtyWarned.has(iso)) return;
      _lockedNoQtyWarned.add(iso);
      if (typeof console !== "undefined" && console.warn) {
        console.warn(`[frame-scheduler] locked week ${iso}${pn ? " (" + pn + ")" : ""} has no stored qty map — live-sizing it this pass; the browser will persist that sizing once and it freezes from then on`);
      }
    }
    ...
    function simulate(rows, cols, slots, globalCaps, rateByPn) {
      ...
      for (let i = 0; i < cols.length; i++) {
        ...
        // VERBATIM REPLAY GATE (slots path). Mirrored at the weekly path
        // below and in js/25 _fsDebugSim -- change all three together.
        // LOCKS FREEZE THE NUMBERS. Any LOCKED row -- manual, weekly-auto,
        // seed -- with a non-empty stored qty map replays that map at ANY
        // horizon distance. Once a week is inside the lock window its
        // stored map IS the schedule: cap edits, on-hand moves, burn
        // changes and catch-up shape only unlocked / proposed weeks. The
        // only way a locked week's numbers change is an operator editing,
        // unpinning or re-picking it.
        // A locked row with NO stored map (possible under v5, which never
        // read stored qty on future weeks) live-sizes here; the browser
        // persists that sizing ONCE (js/25 backfill) and it replays from
        // then on. Pure math cannot persist, so this only says so once.
        const hasStoredQty = !!(wk.qty && Object.keys(wk.qty).length > 0);
        if (slot && slot.locked && !hasStoredQty) _warnLockedNoQty(iso, slot.resolvedPn || slot.pn);
    ...
    function simulateWeekly(rows, cols, slots, globalCaps, rateByPn) {
      ...
      for (let i = 0; i < cols.length; i++) {
        ...
        // VERBATIM REPLAY GATE (weekly path). Mirrors the slots path
        // above and js/25 _fsDebugSim -- change all three together.
        // LOCKS FREEZE THE NUMBERS. Any pinned week with a non-empty
        // stored qty map replays it at any distance.
        //
        // WHAT "LOCKED" MEANS HERE. Weekly-auto rows are persisted with
        // locked:false (only manual pins carry locked:true), so the flag
        // cannot be the test. A weekly row EXISTS for exactly two
        // reasons: the near-term persister wrote it when the week
        // crossed into LOCK_HORIZON_DAYS, or an operator pinned it. Both
        // are the lock. Unpin / re-pick remove the row, which is the only
        // sanctioned way its numbers change. So the presence of a
        // pinByIso entry with a stored map IS "locked with numbers".
        // (fromSlot entries are positionally locked slots; same rule.)
        // The current week already replayed any pinned row's map, so
        // its behaviour is unchanged.
        // A pinned row with no map live-sizes; js/25 backfills the
        // persist once, after which it replays.
        const pinHere = pinByIso.get(iso) || null;
        const hasStoredQty = !!(wk.qty && Object.keys(wk.qty).length > 0);
        if (pinHere && !hasStoredQty) _warnLockedNoQty(iso, pinHere.pn);
        const isLockedWithPersistedQty = !!pinHere && hasStoredQty;
