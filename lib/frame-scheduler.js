...
(function (root, factory) {
  ...
  function forContext(ctx) {
    ...
    function weeklyBaselineHorizonProjection(rows, onHand, weeklyBurnByPn, remainingWeeks) {
      rows = normalizeRows(rows);
    ...
    function weeklyPickAssignment(rows, onHand, previousPn, weeklyBurnByPn, bufWeeksLocal, globalCaps, cols, currentColIdx) {
      ...
      const baselineProjection = weeklyBaselineHorizonProjection(rows, onHand, weeklyBurnByPn, remainingWeeks);
      const candidates = weeklyEnumerateCandidates(rows, onHand, weeklyBurnByPn, globalCaps);

      // Score every candidate up front, then sort with a fully-
      // specified comparator so ties NEVER fall through to insertion
      // (a.k.a. environment) order. This was the fsCompareShadow
      // divergence: the browser and the compute function saw the
      // same score / horizonMinRaw / frames / primary qty and then
      // silently picked whichever the local Map / Object iteration
      // handed them first. Fixed order tuple:
      //   score          desc   (higher score wins)
      //   horizonMinRaw  desc   (higher end-of-horizon min wins)
      //   frames         asc    (fewer distinct frames wins)
      //   primary qty    desc   (larger primary run wins)
      //   primary pn     asc    (stable tie-break by pn)
      //   secondary pn   asc    (final stable tie-break)
      //
      // WEEKLY_TIEBREAK_EPS is applied on `score` only: two scores
      // within eps of each other are treated as equal for the sort
      // key so downstream tuple fields can decide.
      const scored = candidates.map(cand => {
        ...
        // Snap score to eps-buckets so `>` and `===` on the snapped
        // value give the same answer everywhere. Otherwise a float
        // in the last ulp could sort differently between engines.
