...
(function (root, factory) {
  ...
  function forContext(ctx) {
    ...
    // Warn-once for a manual pin that carries no stored qty map. Such a
    // pin cannot replay verbatim (there is nothing to replay) and falls
    // back to live sizing -- which silently un-agrees the numbers with
    // the supplier. Say so, once per week-iso, in both browser and Node.
    // Module-level so the optimizer's tens of thousands of simulate()
    // calls do not repeat it. Pure-math contract kept: console only.
    const _manualPinNoQtyWarned = new Set();
    function _warnManualPinNoQty(iso, pn) {
      if (_manualPinNoQtyWarned.has(iso)) return;
      _manualPinNoQtyWarned.add(iso);
      if (typeof console !== "undefined" && console.warn) {
        console.warn(`[frame-scheduler] manual pin at ${iso}${pn ? " (" + pn + ")" : ""} has no stored qty map — live-sizing it; re-pin to store the agreed quantities`);
      }
    }
    ...
    function simulate(rows, cols, slots, globalCaps, rateByPn) {
      ...
      for (let i = 0; i < cols.length; i++) {
        ...
        //   * MANUAL pin (source "manual", locked) with a stored qty map:
        //     replay at ANY horizon distance. A manual pin is an agreement
        //     with the supplier; the numbers are part of the pin and must
        //     not float with caps / on-hand / burn until the week turns
        //     current.
        //   * auto / seed rows: replay only when current (unchanged).
        //   * manual pin with NO stored qty: fall back to live sizing and
        //     say so once -- the pin is missing the thing that makes it
        //     an agreement.
        const hasStoredQty = !!(wk.qty && Object.keys(wk.qty).length > 0);
        const isManualPin = !!(slot && slot.locked && slot.source === "manual");
        if (isManualPin && !hasStoredQty) _warnManualPinNoQty(iso, slot.resolvedPn || slot.pn);
        const isLockedWithPersistedQty =
          !!slot && slot.locked && hasStoredQty && (c.current || isManualPin);
    ...
    function simulateWeekly(rows, cols, slots, globalCaps, rateByPn) {
      ...
      for (let i = 0; i < cols.length; i++) {
        ...
        // A pinByIso entry is a manual pin here only when it came from a
        // mode:"weekly" slot (fromWeeklyPin) with source "manual" and
        // locked. fromSlot entries are the slots path's business.
        // Weekly-auto pins keep requiring c.current, so they live-size
        // exactly as before.
        const pinHere = pinByIso.get(iso) || null;
        const hasStoredQty = !!(wk.qty && Object.keys(wk.qty).length > 0);
        const isManualPin = !!(pinHere && pinHere.fromWeeklyPin && pinHere.locked && pinHere.source === "manual");
        if (isManualPin && !hasStoredQty) _warnManualPinNoQty(iso, pinHere.pn);
        const isLockedWithPersistedQty =
          !!pinHere && hasStoredQty && (c.current || isManualPin);
